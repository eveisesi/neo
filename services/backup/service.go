...
func (s *service) Run(gLimit, gSleep int64) {
	...
	for {
		count, err := s.redis.ZCount(neo.QUEUES_KILLMAIL_BACKUP, "-inf", "+inf").Result()
		if err != nil {
		...
		results, err := s.redis.ZPopMax(neo.QUEUES_KILLMAIL_BACKUP, gLimit).Result()
		if err != nil {
...
func (s *service) uploadMessage(message []byte, workerID int, sleep int64) {
	...
	if err != nil {
		...
		s.redis.ZAdd(neo.QUEUES_KILLMAIL_BACKUP, &redis.Z{Score: float64(envelope.ID), Member: string(message)})
		return
	}
