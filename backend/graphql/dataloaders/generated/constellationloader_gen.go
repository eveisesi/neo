...
type ConstellationLoaderConfig struct {
	...
	Fetch func(keys []uint) ([]*neo.Constellation, []error)
...
type ConstellationLoader struct {
	...
	cache map[uint]*neo.Constellation
...
func (l *ConstellationLoader) Load(key uint) (*neo.Constellation, error) {
...
func (l *ConstellationLoader) LoadThunk(key uint) func() (*neo.Constellation, error) {
	l.mu.Lock()
...
func (l *ConstellationLoader) LoadAll(keys []uint) ([]*neo.Constellation, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *ConstellationLoader) LoadAllThunk(keys []uint) func() ([]*neo.Constellation, []error) {
	results := make([]func() (*neo.Constellation, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *ConstellationLoader) Prime(key uint, value *neo.Constellation) bool {
	l.mu.Lock()
	var found bool
...
func (l *ConstellationLoader) unsafeSet(key uint, value *neo.Constellation) {
	if l.cache == nil {
		l.cache = map[uint]*neo.Constellation{}
	}
	l.cache[key] = value
}
