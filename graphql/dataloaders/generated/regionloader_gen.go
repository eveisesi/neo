...
type RegionLoaderConfig struct {
	...
	Fetch func(keys []uint64) ([]*neo.Region, []error)
...
func (l *RegionLoader) Load(key uint64) (*neo.Region, error) {
...
func (l *RegionLoader) LoadThunk(key uint64) func() (*neo.Region, error) {
	l.mu.Lock()
...
func (l *RegionLoader) LoadAll(keys []uint64) ([]*neo.Region, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *RegionLoader) LoadAllThunk(keys []uint64) func() ([]*neo.Region, []error) {
	results := make([]func() (*neo.Region, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *RegionLoader) Prime(key uint64, value *neo.Region) bool {
	l.mu.Lock()
	var found bool
...
func (l *RegionLoader) Clear(key uint64) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *RegionLoader) unsafeSet(key uint64, value *neo.Region) {
	if l.cache == nil {
		l.cache = map[uint64]*neo.Region{}
	}
	l.cache[key] = value
}
...
func (b *regionLoaderBatch) keyIndex(l *RegionLoader, key uint64) int {
