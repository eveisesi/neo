...
type ConstellationLoaderConfig struct {
	...
	Fetch func(keys []uint64) ([]*neo.Constellation, []error)
...
type ConstellationLoader struct {
	...
	cache map[uint64]*neo.Constellation
...
func (l *ConstellationLoader) Load(key uint64) (*neo.Constellation, error) {
...
func (l *ConstellationLoader) LoadThunk(key uint64) func() (*neo.Constellation, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*neo.Constellation, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *ConstellationLoader) LoadAll(keys []uint64) ([]*neo.Constellation, []error) {
	results := make([]func() (*neo.Constellation, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	constellations := make([]*neo.Constellation, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *ConstellationLoader) LoadAllThunk(keys []uint64) func() ([]*neo.Constellation, []error) {
	results := make([]func() (*neo.Constellation, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *ConstellationLoader) Prime(key uint64, value *neo.Constellation) bool {
	l.mu.Lock()
	var found bool
...
func (l *ConstellationLoader) Clear(key uint64) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *ConstellationLoader) unsafeSet(key uint64, value *neo.Constellation) {
	if l.cache == nil {
		l.cache = map[uint64]*neo.Constellation{}
	}
	l.cache[key] = value
}
...
func (b *constellationLoaderBatch) keyIndex(l *ConstellationLoader, key uint64) int {
