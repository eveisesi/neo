...
type KillmailItemsLoaderConfig struct {
	...
	Fetch func(keys []uint) ([][]*neo.KillmailItem, []error)
...
type KillmailItemsLoader struct {
	...
	cache map[uint][]*neo.KillmailItem
...
func (l *KillmailItemsLoader) Load(key uint) ([]*neo.KillmailItem, error) {
...
func (l *KillmailItemsLoader) LoadThunk(key uint) func() ([]*neo.KillmailItem, error) {
	l.mu.Lock()
...
func (l *KillmailItemsLoader) LoadAll(keys []uint) ([][]*neo.KillmailItem, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *KillmailItemsLoader) LoadAllThunk(keys []uint) func() ([][]*neo.KillmailItem, []error) {
	results := make([]func() ([]*neo.KillmailItem, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *KillmailItemsLoader) Prime(key uint, value []*neo.KillmailItem) bool {
	l.mu.Lock()
	var found bool
...
func (l *KillmailItemsLoader) Clear(key uint) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *KillmailItemsLoader) unsafeSet(key uint, value []*neo.KillmailItem) {
	if l.cache == nil {
		l.cache = map[uint][]*neo.KillmailItem{}
	}
	l.cache[key] = value
}
...
func (b *killmailItemsLoaderBatch) keyIndex(l *KillmailItemsLoader, key uint) int {
