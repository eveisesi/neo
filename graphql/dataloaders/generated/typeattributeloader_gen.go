...
type TypeAttributeLoaderConfig struct {
	...
	Fetch func(keys []uint) ([][]*neo.TypeAttribute, []error)
...
type TypeAttributeLoader struct {
	...
	cache map[uint][]*neo.TypeAttribute
...
func (l *TypeAttributeLoader) Load(key uint) ([]*neo.TypeAttribute, error) {
...
func (l *TypeAttributeLoader) LoadThunk(key uint) func() ([]*neo.TypeAttribute, error) {
	l.mu.Lock()
...
func (l *TypeAttributeLoader) LoadAll(keys []uint) ([][]*neo.TypeAttribute, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *TypeAttributeLoader) LoadAllThunk(keys []uint) func() ([][]*neo.TypeAttribute, []error) {
	results := make([]func() ([]*neo.TypeAttribute, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *TypeAttributeLoader) Prime(key uint, value []*neo.TypeAttribute) bool {
	l.mu.Lock()
	var found bool
...
func (l *TypeAttributeLoader) unsafeSet(key uint, value []*neo.TypeAttribute) {
	if l.cache == nil {
		l.cache = map[uint][]*neo.TypeAttribute{}
	}
	l.cache[key] = value
}
...
func (b *typeAttributeLoaderBatch) keyIndex(l *TypeAttributeLoader, key uint) int {
