...
func (o *Faction) KillmailVictims(mods ...qm.QueryMod) killmailVictimQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`killmail_victim`.`faction_id`=?", o.ID),
	)
...
func (factionL) LoadKillmailVictims(ctx context.Context, e boil.ContextExecutor, singular bool, maybeFaction interface{}, mods queries.Applicator) error {
	var slice []*Faction
	var object *Faction
	...
	query := NewQuery(qm.From(`killmail_victim`), qm.WhereIn(`killmail_victim.faction_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Faction) AddKillmailVictims(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*KillmailVictim) error {
	var err error
...
// Sets o.R.Faction's KillmailVictims accordingly.
// Replaces o.R.KillmailVictims with related.
// Sets related.R.Faction's KillmailVictims accordingly.
func (o *Faction) SetKillmailVictims(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*KillmailVictim) error {
	query := "update `killmail_victim` set `faction_id` = null where `faction_id` = ?"
...
func (o *Faction) RemoveKillmailVictims(ctx context.Context, exec boil.ContextExecutor, related ...*KillmailVictim) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.FactionID, nil)
		if rel.R != nil {
		...
		if _, err = rel.Update(ctx, exec, boil.Whitelist("faction_id")); err != nil {
			return err
		}
	}
	if o.R == nil {
