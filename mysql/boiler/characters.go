...
func (o *Character) KillmailAttackers(mods ...qm.QueryMod) killmailAttackerQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`killmail_attackers`.`character_id`=?", o.ID),
	)
...
func (o *Character) KillmailVictims(mods ...qm.QueryMod) killmailVictimQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`killmail_victim`.`character_id`=?", o.ID),
	)
...
func (characterL) LoadKillmailAttackers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCharacter interface{}, mods queries.Applicator) error {
	var slice []*Character
	...
	query := NewQuery(qm.From(`killmail_attackers`), qm.WhereIn(`killmail_attackers.character_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (characterL) LoadKillmailVictims(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCharacter interface{}, mods queries.Applicator) error {
	var slice []*Character
	var object *Character
	...
	query := NewQuery(qm.From(`killmail_victim`), qm.WhereIn(`killmail_victim.character_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Character) AddKillmailAttackers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*KillmailAttacker) error {
	var err error
...
// character replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Character's KillmailAttackers accordingly.
// Replaces o.R.KillmailAttackers with related.
// Sets related.R.Character's KillmailAttackers accordingly.
func (o *Character) SetKillmailAttackers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*KillmailAttacker) error {
	query := "update `killmail_attackers` set `character_id` = null where `character_id` = ?"
...
// Sets related.R.Character.
func (o *Character) RemoveKillmailAttackers(ctx context.Context, exec boil.ContextExecutor, related ...*KillmailAttacker) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.CharacterID, nil)
		if rel.R != nil {
		...
		if _, err = rel.Update(ctx, exec, boil.Whitelist("character_id")); err != nil {
			return err
		}
	}
	if o.R == nil {
...
// Sets related.R.Character appropriately.
func (o *Character) AddKillmailVictims(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*KillmailVictim) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.CharacterID, o.ID)
...
// character replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Character's KillmailVictims accordingly.
// Replaces o.R.KillmailVictims with related.
// Sets related.R.Character's KillmailVictims accordingly.
func (o *Character) SetKillmailVictims(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*KillmailVictim) error {
	query := "update `killmail_victim` set `character_id` = null where `character_id` = ?"
	values := []interface{}{o.ID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
...
// Sets related.R.Character.
func (o *Character) RemoveKillmailVictims(ctx context.Context, exec boil.ContextExecutor, related ...*KillmailVictim) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.CharacterID, nil)
		if rel.R != nil {
