...
var TypeFlagRels = struct {
	FlagKillmailItems string
}{
	FlagKillmailItems: "FlagKillmailItems",
}

// typeFlagR is where relationships are stored.
type typeFlagR struct {
	FlagKillmailItems KillmailItemSlice
}
...
// FlagKillmailItems retrieves all the killmail_item's KillmailItems with an executor via flag_id column.
func (o *TypeFlag) FlagKillmailItems(mods ...qm.QueryMod) killmailItemQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`killmail_items`.`flag_id`=?", o.ID),
	)
...
// LoadFlagKillmailItems allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (typeFlagL) LoadFlagKillmailItems(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTypeFlag interface{}, mods queries.Applicator) error {
	var slice []*TypeFlag
	var object *TypeFlag

	if singular {
		object = maybeTypeFlag.(*TypeFlag)
	} else {
		slice = *maybeTypeFlag.(*[]*TypeFlag)
	}
	...
	query := NewQuery(qm.From(`killmail_items`), qm.WhereIn(`killmail_items.flag_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.FlagKillmailItems = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &killmailItemR{}
			}
			foreign.R.Flag = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.FlagID {
				local.R.FlagKillmailItems = append(local.R.FlagKillmailItems, foreign)
				if foreign.R == nil {
					foreign.R = &killmailItemR{}
				}
				foreign.R.Flag = local
				break
			}
		}
	}
...
// AddFlagKillmailItems adds the given related objects to the existing relationships
// of the type_flag, optionally inserting them as new records.
// Appends related to o.R.FlagKillmailItems.
// Sets related.R.Flag appropriately.
func (o *TypeFlag) AddFlagKillmailItems(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*KillmailItem) error {
	var err error
	...
	if o.R == nil {
		o.R = &typeFlagR{
			FlagKillmailItems: related,
		}
	} else {
		o.R.FlagKillmailItems = append(o.R.FlagKillmailItems, related...)
	}
